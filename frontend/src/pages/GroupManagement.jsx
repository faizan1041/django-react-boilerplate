...
const GroupManagement = () => {
  ...
  const createGroupForm = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        // Create the group
        const response = await groupService.create({ name: values.name });
  ...
  const editGroupForm = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // Update group name
        await groupService.update(values.id, { name: values.name });
  ...
  const groupPermissionsByApp = () => {
    ...
    permissions.forEach(permission => {
      ...
      groupedPermissions[app].push(permission);
    });
          ...
          <form onSubmit={createGroupForm.handleSubmit}>
            <div className="grid grid-cols-6 gap-6">
              ...
              <div className="col-span-6">
                ...
                <div className="mt-2 border border-gray-200 rounded-md p-4 h-64 overflow-y-auto">
                  {Object.entries(groupPermissionsByApp()).map(([app, appPermissions]) => (
